...
class ComponentIntegrationTest {
      ...
      try {
        ...
        imports.forEach(importStatement => {
          ...
          if (isValid) {
            ...
            this.addResult(`Import in ${fileName}`, 'failed', `Invalid import: ${importStatement}`);
          }
        });
        
      } catch (error) {
        this.addResult(path.basename(filePath), 'failed', 'Error reading file', error.message);
      }
    });
    
    const importSuccessRate = Math.round((validImports / totalImports) * 100);
    if (importSuccessRate >= 95) {
      this.addResult('Import Validation', 'passed', `${importSuccessRate}% of imports are valid`);
    } else {
      this.addResult('Import Validation', 'failed', `Only ${importSuccessRate}% of imports are valid`);
    }
  }
    ...
    this.componentPaths.forEach(filePath => {
      try {
        ...
        if (content.includes('export default')) {
          ...
          this.addResult(`${fileName} Export`, 'warnings', 'No default export found');
        }
      ...
      try {
        ...
        if (content.includes('useTheme') || content.includes('ThemeProvider')) {
          ...
          if (content.includes('theme.colors')) {
            this.addResult(`${fileName} Theme`, 'passed', 'Properly uses theme colors');
          } else {
            this.addResult(`${fileName} Theme`, 'warnings', 'Imports theme but may not use colors');
          }
        }
        
      } catch (error) {
        this.addResult(path.basename(filePath), 'failed', 'Error checking theme integration');
      }
    });
    
    const themePercentage = Math.round((themeAwareFiles / allFiles.length) * 100);
    this.addResult('Theme Coverage', 'passed', `${themePercentage}% of files use theme system`);
  }
  ...
  testIconIntegration() {
    ...
    allFiles.forEach(filePath => {
      try {
        ...
        if (content.includes('MentalHealthIcon') || content.includes('ActionIcon') || content.includes('NavigationIcon')) {
          iconAwareFiles++;
          this.addResult(`${fileName} Icons`, 'passed', 'Uses modern icon system');
        } else if (content.includes('MaterialIcons') || content.includes('Ionicons')) {
          this.addResult(`${fileName} Icons`, 'warnings', 'Uses legacy icon system');
        }
        
      } catch (error) {
        this.addResult(path.basename(filePath), 'failed', 'Error checking icon integration');
      }
    });
    
    if (iconAwareFiles > 0) {
      this.addResult('Icon System', 'passed', `${iconAwareFiles} files use modern icon system`);
    } else {
      this.addResult('Icon System', 'warnings', 'No files using modern icon system');
    }
  }
    ...
    allFiles.forEach(filePath => {
      try {
        ...
        if (accessibilityScore >= 2) {
          accessibleFiles++;
          this.addResult(`${fileName} A11y`, 'passed', `Good accessibility support (${accessibilityScore}/${maxScore})`);
        } else if (accessibilityScore > 0) {
          this.addResult(`${fileName} A11y`, 'warnings', `Basic accessibility support (${accessibilityScore}/${maxScore})`);
        }
        
      } catch (error) {
        this.addResult(path.basename(filePath), 'failed', 'Error checking accessibility');
      }
    });
    
    const a11yPercentage = Math.round((accessibleFiles / allFiles.length) * 100);
    if (a11yPercentage >= 70) {
      this.addResult('Accessibility Coverage', 'passed', `${a11yPercentage}% of files have good accessibility`);
    } else {
      this.addResult('Accessibility Coverage', 'warnings', `Only ${a11yPercentage}% of files have good accessibility`);
    }
  }
    ...
    try {
      ...
      designSystemFiles.forEach(file => {
        ...
        if (fs.existsSync(filePath)) {
          this.addResult(path.basename(file), 'passed', 'Design system file exists');
        } else {
          this.addResult(path.basename(file), 'failed', 'Design system file missing');
        }
      });
      ...
      if (appContent.includes('DesignSystemProvider')) {
        this.addResult('App Integration', 'passed', 'DesignSystemProvider integrated in App.js');
      } else {
        this.addResult('App Integration', 'failed', 'DesignSystemProvider not integrated in App.js');
      }
      
    } catch (error) {
      this.addResult('Design System', 'failed', 'Error testing design system integration');
    }
  }
  ...
  printSummary() {
    ...
    try {
      const reportPath = path.join(__dirname, 'component-integration-report.json');
