...
(async () => {
  ...
  try {
    ...
    if (rootElement) {
      ...
      console.log('Total non-script/style elements in root:', allElements.length);
      ...
      for (let i = 0; i < Math.min(10, allElements.length); i++) {
        ...
        const text = await element.evaluate(el => el.textContent?.substring(0, 50));
    ...
    const navElements = await page.$$('nav, [role="navigation"], [role="tablist"]');
    ...
    const reactElements = await page.$$('[data-reactroot], [data-react-helmet], [data-testid]');
    console.log('Elements with React attributes:', reactElements.length);

    // Check for loading indicators
    const loadingElements = await page.$$('text=/loading/i, text=/loading/i, *[class*="loading"], *[class*="Loading"]');
    console.log('Loading elements found:', loadingElements.length);

    // Check for error messages
    const errorElements = await page.$$('text=/error/i, text=/Error/i, *[class*="error"], *[class*="Error"]');
    ...
    const isLoading = await page.evaluate(() => {
      const loadingIndicators = [
        ...
        'initializing', 'Initializing', 'connecting', 'Connecting'
      ];
