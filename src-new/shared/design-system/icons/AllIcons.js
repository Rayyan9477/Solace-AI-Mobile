...
export const IconCategories = {
  // Health & Medical
  'Brain & Mental': [
    'brain', 'mind', 'neuron', 'mindfulness', 'meditation-pose', 
    'zen-circle', 'emotional-balance', 'inner-peace', 'emotional-growth'
  ],
  
  'Physical Health': [
    'heart-pulse', 'wellness', 'stethoscope', 'health-monitor', 
    'thermometer', 'activity', 'exercise', 'nutrition', 'water'
  ],
  
  'Therapy & Support': [
    'therapy', 'counseling', 'treatment', 'therapy-session', 
    'support-group', 'counselor', 'crisis-support', 'emergency-contact'
  ],
  
  'Wellness & Self-Care': [
    'self-care', 'wellness-routine', 'mental-strength', 'sleep', 
    'recovery', 'breathing-exercise', 'relaxation'
  ],
  
  'Mood & Emotions': [
    'mood-tracker', 'emotions', 'feelings-journal', 'progress-tracking', 'mental-goals'
  ],
  
  // UI & Interface
  'Basic Controls': [
    'menu', 'close', 'search', 'filter', 'sort', 'more-horizontal', 
    'more-vertical', 'plus', 'minus'
  ],
  
  'Actions': [
    'edit', 'delete', 'save', 'copy', 'check', 'check-circle', 
    'x-circle', 'alert-circle', 'info'
  ],
  
  'View Controls': [
    'eye', 'eye-off', 'grid', 'list', 'zoom-in', 'zoom-out'
  ],
  ...
  // Navigation & Layout
  'Main Navigation': [
    'home', 'dashboard', 'profile', 'chat', 'explore', 'discover'
  ],
  
  'Layout & Structure': [
    'layout-grid', 'layout-list', 'layout-columns', 'layout-rows', 
    'sidebar', 'modal', 'drawer', 'popup'
  ],
  
  'Screen States': [
    'fullscreen', 'minimize', 'split-screen', 'expand', 'collapse'
  ],
  ...
  'Circular & Special': [
    ...
    'navigation', 'compass', 'move', 'drag'
  ],
  
  // Files & Documents
  'File System': [
    'folder', 'file', 'document', 'settings', 'preferences'
  ],
  
  // Data Visualization
  'Charts & Analytics': [
    'bar-chart', 'line-chart', 'area-chart', 'pie-chart', 'donut-chart',
    'scatter-plot', 'analytics', 'trending-up', 'trending-down'
  ],
  
  'Progress & Metrics': [
    'progress-ring', 'progress-bar', 'statistics', 'metrics', 
    'dashboard-data', 'compare', 'correlation'
  ],
  
  'Mental Health Data': [
    'mood-graph', 'wellness-meter', 'stress-level', 'heart-rate-chart', 'sleep-chart'
  ],
  
  // Accessibility & Communication
  'Accessibility': [
    'accessibility', 'wheelchair', 'hearing-aid', 'sign-language', 
    'braille', 'voice-recognition', 'screen-reader', 'high-contrast'
  ],
  
  'Communication': [
    'chat-bubble', 'speech-bubble', 'video-call', 'voice-call',
    'text-to-speech', 'live-caption', 'translation'
  ],
  
  'Support & Help': [
    'help-circle', 'support', 'feedback', 'contact-support',
    'language', 'localization'
  ],
  
  // Notification & Status
  'Notifications': [
    'bell', 'bell-off', 'notification', 'notification-dot',
    'message-notification', 'reminder-notification', 'therapy-reminder'
  ],
  
  'Status Indicators': [
    'alert', 'alert-circle', 'warning', 'error', 'success', 'info',
    'status-online', 'status-offline', 'status-away', 'status-busy'
  ],
  
  'Progress & Loading': [
    'loading', 'spinner', 'progress-indicator', 'badge', 'badge-check'
  ],
  
  'Mental Health Status': [
    'mood-positive', 'mood-negative', 'mood-neutral', 'wellness-status'
  ],
  
  'Priority & Urgency': [
    'priority-high', 'priority-medium', 'priority-low', 'urgent'
  ],
};
...
export const TherapeuticThemeMapping = {
  ...
  calming: [
    'breathing-exercise', 'relaxation', 'zen-circle', 'inner-peace',
    'meditation-pose', 'mindfulness'
  ],
  
  // Nurturing themes for growth and healing
  nurturing: [
    'emotional-growth', 'self-care', 'wellness-routine', 'support-group',
    'counselor', 'therapy-session'
  ],
  
  // Peaceful themes for serenity and balance
  peaceful: [
    'emotional-balance', 'mental-strength', 'progress-tracking',
    'wellness', 'sleep'
  ],
  
  // Grounding themes for stability
  grounding: [
    'therapy', 'treatment', 'counseling', 'mental-goals',
    'crisis-support'
  ],
  
  // Energizing themes for motivation
  energizing: [
    'exercise', 'activity', 'mood-tracker', 'emotions'
  ],
  
  // Focus themes for concentration
  focus: [
    'brain', 'mind', 'neuron', 'search', 'filter', 'zoom-in',
    'analytics', 'bar-chart', 'line-chart', 'dashboard-data'
  ],
  
  // Mindful themes for awareness
  mindful: [
    'feelings-journal', 'heart-pulse', 'nutrition', 'water',
    'mood-graph', 'wellness-meter', 'mood-positive', 'mood-neutral'
  ],
  
  // Balance themes for equilibrium
  balance: [
    'home', 'dashboard', 'settings', 'preferences',
    'notification', 'reminder-notification', 'wellness-status',
    'priority-medium', 'badge', 'status-online'
  ],
};
...
export const IconUsageExamples = {
  ...
  therapeutic: {
    ...
    usage: '<MindfulnessIcon size={32} therapeuticTheme="calming" variant="filled" />',
  },
  ...
  interactive: {
    ...
    usage: '<HeartPulseIcon size={28} therapeuticTheme="nurturing" style={{ animationDuration: "2s" }} />',
  },
};
...
export const IconUtils = {
  ...
  validateIconProps: (props) => {
    ...
    if (props.size && props.size < IconAccessibilityGuidelines.minimumTouchTarget) {
      warnings.push(`Icon size ${props.size}px is below minimum touch target of ${IconAccessibilityGuidelines.minimumTouchTarget}px`);
    }
    
    if (!props.accessibilityLabel && !props.testID) {
      warnings.push('Consider adding accessibilityLabel or testID for better accessibility');
    }
