  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        if (refreshToken) {
          const response = await axios.post(`${currentConfig.baseURL}/auth/refresh`, {
...
export const apiService = {
  ...
  auth: {
    ...
    async resetPassword(token, newPassword) {
      return await apiClient.post('/auth/reset-password', { token, password: newPassword });
    },
  },
  ...
  assessments: {
    ...
    async getHistory(limit = 10) {
      const response = await apiClient.get(`/assessments/history?limit=${limit}`);
      return response.data;
    },
