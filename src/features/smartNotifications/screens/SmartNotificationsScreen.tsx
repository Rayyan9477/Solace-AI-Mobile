...
interface Notification {
  id: string;
  type: 'message' | 'journal' | 'exercise' | 'health' | 'mood' | 'stress' | 'recommendation';
  title: string;
  message: string;
  time: string;
  isNew: boolean;
...
const NOTIFICATION_CARDS = [
  ...
  },
  {
    ...
    subtitle: 'You still need to complete 4 daily\njournal this month. Keep it up! 🎉',
    action: 'See Journal',
    scoreValue: '21/30',
    illustration: '📖',
...
export const SmartNotificationsScreen = ({ navigation }: any) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        {viewMode === 'list' ? renderNotificationList() : renderNotificationCards()}
      </ScrollView>
    </SafeAreaView>
  );
};
