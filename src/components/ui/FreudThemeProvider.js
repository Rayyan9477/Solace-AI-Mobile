...
const buildMaterialTheme = (therapeuticTheme, isDark = false) => {
  const therapeutic =
    THERAPEUTIC_THEMES[therapeuticTheme] || THERAPEUTIC_THEMES.balanced;
...
export const FreudThemeProvider = ({
  ...
  const getMoodBasedColors = useMemo(
    () => (currentMood) => {
  ...
  const materialTheme = useMemo(() => {
    const activeTherapeutic = mood
      ? Object.keys(THERAPEUTIC_THEMES).find(
          (t) => THERAPEUTIC_THEMES[t].primary === MOOD_COLORS[mood]?.[60],
        ) || therapeutic
      : therapeutic;
