...
const CrisisRiskAssessment = ({ visible, onClose, onRiskLevelDetermined }) => {
  ...
  const handleResponse = useCallback((questionId, response) => {
    ...
    crisisQuestions.forEach(question => {
      ...
      if (response !== undefined) {
        ...
        if (question.type === 'yes_no') {
          ...
          if (question.inverted) score = response ? 0 : Math.abs(question.weight);
  ...
  const handleCrisisIntervention = (riskLevel) => {
    const interventions = {
      critical: {
        ...
        message: 'Your responses indicate you may be in crisis. Please reach out for immediate support.',
        actions: [
          {
      ...
      high: {
        ...
        message: 'Your responses suggest you may be at elevated risk. Consider reaching out to a mental health professional.',
        actions: [
          {
      ...
      moderate: {
        ...
        message: 'Your responses indicate some concerning thoughts. Consider speaking with a counselor or trusted person.',
        actions: [
          {
      ...
      <View style={[styles.container, { backgroundColor: theme.colors.background.primary }]}>
        ...
        >
          <MentalHealthIcon
            ...
            colorScheme="error"
          />
          <Text style={[styles.headerTitle, { color: theme.colors.error[700] }]}>
            Safety Check
          </Text>
          <Text style={[styles.headerSubtitle, { color: theme.colors.error[600] }]}>
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.questionContainer}>
            ...
            {currentQ.type === 'yes_no' && (
              <View style={styles.yesNoContainer}>
                <TouchableOpacity
                  style={[
                    styles.yesNoButton,
                    { backgroundColor: theme.colors.error[100], borderColor: theme.colors.error[500] },
                  ]}
                ...
                >
                  <Text style={[styles.yesNoText, { color: theme.colors.error[700] }]}>
                    Yes
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[
                    styles.yesNoButton,
                    { backgroundColor: theme.colors.success[100], borderColor: theme.colors.success[500] },
                  ]}
                ...
                >
                  <Text style={[styles.yesNoText, { color: theme.colors.success[700] }]}>
                    No
                  </Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.scaleContainer}>
                {currentQ.scale.map((label, index) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.scaleButton,
                      { backgroundColor: theme.colors.gray[100], borderColor: theme.colors.gray[300] },
                    ]}
            ...
            <View style={[styles.crisisBox, { backgroundColor: theme.colors.error[50] }]}>
              <Text style={[styles.crisisTitle, { color: theme.colors.error[700] }]}>
                🆘 Need Immediate Help?
              </Text>
              <Text style={[styles.crisisText, { color: theme.colors.error[600] }]}>
                If you're in crisis, don't wait - reach out now:
              </Text>
              <TouchableOpacity
                style={[styles.crisisButton, { backgroundColor: theme.colors.error[500] }]}
                onPress={() => Linking.openURL('tel:988')}
                accessible={true}
                accessibilityRole="button"
        ...
        <View style={styles.footer}>
          <TouchableOpacity
            style={[styles.skipButton, { backgroundColor: theme.colors.gray[200] }]}
            onPress={onClose}
            accessible={true}
            accessibilityRole="button"
