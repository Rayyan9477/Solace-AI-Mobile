...
const DANGEROUS_PATTERNS = {
  ...
  // Common XSS patterns
  XSS_PATTERNS:
    /(<|&lt;)[\/]*script[^>]*>|javascript:|vbscript:|on\w+\s*=|<[^>]*style[^>]*=[^>]*expression[^>]*>|<[^>]*style[^>]*=[^>]*behavior[^>]*>/gi,
};
...
export const sanitizeMoodData = (moodData) => {
  ...
  if (
    typeof moodData.mood === "string" &&
    allowedMoods.includes(moodData.mood.toLowerCase())
  ) {
  ...
  // Sanitize notes
  if (moodData.notes) {
    sanitized.notes = sanitizeMentalHealthNotes(moodData.notes, {
      maxLength: 1000,
    });
  }
