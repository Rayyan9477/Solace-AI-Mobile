...
export const useMotionAccessibility = () => {
  ...
  const getAnimationDuration = useCallback(
    (defaultDuration, motionType = MOTION_TYPES.FADE) => {
      if (isReducedMotionEnabled) {
      ...
      return (
        defaultDuration *
        (multipliers[motionPreferences.sensitivityLevel] || 1.0)
      );
    },
    [
      isReducedMotionEnabled,
      isMotionAllowed,
      motionPreferences.sensitivityLevel,
    ],
  );

  // Get safe animation configuration
  const getSafeAnimationConfig = useCallback(
    (config, motionType = MOTION_TYPES.FADE) => {
  ...
  const createSafeTiming = useCallback(
    (animatedValue, toValue, config = {}, motionType = MOTION_TYPES.FADE) => {
  ...
  const createSafeSpring = useCallback(
    (animatedValue, toValue, config = {}, motionType = MOTION_TYPES.BOUNCE) => {
...
export const withMotionAccessibility = (WrappedComponent) => {
  return function MotionAccessibleComponent(props) {
    ...
    return <WrappedComponent {...props} motionUtils={motionUtils} />;
  };
};
...
export const getMotionSettingsProps = (motionUtils) => ({
  respectSystemPreferences: {
    ...
    onValueChange: (value) =>
      motionUtils.updateMotionPreferences({
