...
export class MentalHealthNotificationService {
    ...
    try {
      ...
      if (Platform.OS === "android") {
        await Notifications.setNotificationChannelAsync("mental-health", {
          ...
          description:
            "Gentle reminders for mental health check-ins and self-care",
  ...
  static async scheduleTherapyReminder(
    appointmentTime,
    ...
    try {
      ...
      console.log("Therapy reminders scheduled:", {
        dayBeforeIdentifier,
        hourBeforeIdentifier,
      });
    ...
    try {
      ...
      if (wellnessReminders) {
        ...
        scheduledNotifications.push(
          ...wellnessIds.map((id) => ({ type: "wellness", id })),
        );
      }

      if (assessmentReminders) {
        const assessmentId =
          await this.scheduleAssessmentReminder(assessmentFrequency);
        if (assessmentId)
          scheduledNotifications.push({ type: "assessment", id: assessmentId });
      }
