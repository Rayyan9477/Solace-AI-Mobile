...
export const PaperComponents = {
  ...
  Button: () => import('react-native-paper/lib/module/components/Button/Button').then(m => m.default),
  Card: () => import('react-native-paper/lib/module/components/Card/Card').then(m => m.default),
  Surface: () => import('react-native-paper/lib/module/components/Surface').then(m => m.default),
...
export const LightweightStyling = {
  ...
  platformStyle: (iosStyle, androidStyle, webStyle) => {
    switch (Platform.OS) {
      ...
      case 'web': return webStyle || iosStyle;
      default: return iosStyle;
    }
  }
};
  ...
  trackComponentMount: (componentName) => {
    if (__DEV__) {
      ...
      return () => {
        ...
        console.log(`${componentName} mount time: ${(endTime - startTime).toFixed(2)}ms`);
      };
    }
    return () => {};
  }
