...
export const useOptimizedDashboardData = () => {
  ...
  }), [userData, moodData, chatData, moodHistorySlice, chatHistorySlice, insights]);
};
...
export const useOptimizedChatSessionData = () => {
  ...
  }), [chatData.currentSession, chatData.isTyping, chatData.loading, userData.profile, recentMessages]);
};
...
export const useOptimizedAssessmentSessionData = () => {
  ...
  const progress = useMemo(() => {
    ...
    return Math.round((Object.keys(current.answers).length / current.questions.length) * 100);
