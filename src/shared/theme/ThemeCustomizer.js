...
export const AccessibilityThemes = {
  highContrast: () =>
    new ThemeCustomizer("light").setThemeOverrides({
      colors: {
        text: {
          primary: "#000000",
          secondary: "#333333",
...
export const ThemeValidator = {
  ...
  validateTherapeuticColors: (theme) => {
    ...
    if (
      theme.components?.Button?.therapeuticColor === "energizing" &&
      !theme.components?.Button?.size === "large"
    ) {
      warnings.push(
        "Energizing buttons should typically be large for crisis situations",
      );
    }
