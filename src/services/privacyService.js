    ...
    return {
      ...
      content: {
        ...
        dataRetention: {
          ...
          description:
            "Mental health data is retained for 7 years or until you request deletion, whichever comes first.",
        },
        userRights: {
          title: "Your Rights",
          items: [
      ...
      required: [
        {
          ...
          description:
            "Allow processing of mental health data for therapy services",
          category: "essential",
          required: true,
        },
        {
          ...
          description:
            "Allow emergency contacts and services to be notified during crisis situations",
          category: "safety",
          required: true,
        },
      ],
      optional: [
        {
          ...
          title: "Analytics",
          description:
            "Allow anonymous usage analytics to improve app performance",
        ...
        },
        {
          ...
          description:
            "Receive updates about new features and mental health resources",
  ...
  async deleteUserData(verificationCode = null) {
    try {
      ...
      const userDataKeys = allKeys.filter(
        (key) => !key.startsWith("system_") && !key.startsWith("app_config"),
      );
  ...
  async checkDataRetention() {
    try {
      ...
      const retentionExpiry = new Date(
        createdDate.getTime() + this.dataRetentionPeriod,
      );
      const now = new Date();
      ...
      const daysUntilExpiry = Math.ceil(
        (retentionExpiry - now) / (24 * 60 * 60 * 1000),
      );
...
export const recordConsent = (consentData) =>
  privacyService.recordConsent(consentData);
export const getConsentStatus = () => privacyService.getConsentStatus();
export const hasConsentForAction = (action) =>
  privacyService.hasConsentForAction(action);
export const exportUserData = () => privacyService.exportUserData();
export const deleteUserData = (verificationCode) =>
  privacyService.deleteUserData(verificationCode);
