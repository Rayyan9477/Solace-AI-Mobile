  ...
  const generateAIResponse = useCallback(
    (userInput, context = {}) => {
      ...
      if (currentExercise && context.exerciseStep !== undefined) {
        const exercise = therapyExercises.find(
          (ex) => ex.id === currentExercise,
        );
    ...
    async (messageText) => {
      ...
      setTimeout(() => {
        const context = currentExercise
          ? {
              exerciseStep: messages.filter((m) => m.type === "exercise_prompt")
                .length,
            }
          : {};
