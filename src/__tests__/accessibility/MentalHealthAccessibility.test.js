...
const AccessibilityTestWrapper = ({ children, isScreenReaderEnabled = false, isReducedMotion = false }) => {
  AccessibilityInfo.isScreenReaderEnabled.mockResolvedValue(isScreenReaderEnabled);
...
describe('Mental Health Accessibility', () => {
    ...
    describe('Touch Target Size (WCAG 2.5.5)', () => {
      it('ensures all interactive elements meet 44x44px minimum', () => {
        ...
        buttons.forEach(button => {
          const { width, height, minWidth, minHeight } = button.props.style || {};
          const actualWidth = minWidth || width || WCAG_CONSTANTS.MIN_TOUCH_TARGET;
          const actualHeight = minHeight || height || WCAG_CONSTANTS.MIN_TOUCH_TARGET;
          
          expect(actualWidth).toBeGreaterThanOrEqual(WCAG_CONSTANTS.MIN_TOUCH_TARGET);
          expect(actualHeight).toBeGreaterThanOrEqual(WCAG_CONSTANTS.MIN_TOUCH_TARGET);
        });
      });
      ...
      it('provides adequate spacing between touch targets', () => {
        ...
        buttons.forEach(button => {
          expect(button.props.style?.margin || button.props.style?.padding).toBeTruthy();
        });
      });
    });
  ...
  describe('Screen Reader Support', () => {
    it('provides comprehensive screen reader navigation', async () => {
      const { getByTestId } = render(
        <AccessibilityTestWrapper isScreenReaderEnabled={true}>
          <MainAppScreen />
        </AccessibilityTestWrapper>
      );
  ...
  describe('Mental Health Specific Accessibility', () => {
    it('provides crisis-accessible emergency features', () => {
      ...
      const crisisElements = screen.queryAllByText(/crisis|emergency|help|support/i);
    ...
    it('supports users in distress with gentle interactions', () => {
      ...
      expect(component.props.accessibilityLabel).not.toMatch(/wrong|bad|negative/);
    });
  ...
  describe('Reduced Motion Support', () => {
    it('respects reduced motion preferences', () => {
      const { getByTestId } = render(
        <AccessibilityTestWrapper isReducedMotion={true}>
          <MoodCheckIn testID="mood-check-in" />
        </AccessibilityTestWrapper>
      );
