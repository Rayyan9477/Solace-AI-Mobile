...
describe('CrisisManager', () => {
    ...
    it('detects suicide-related keywords', () => {
      ...
      suicidalTexts.forEach(text => {
        ...
        expect(result.keywords).toContain(
          expect.stringMatching(/suicide|kill myself|end my life|better off dead|no point/i)
        );
      });
    });
  ...
  describe('Error Handling and Resilience', () => {
    it('handles network failures gracefully', async () => {
      ...
      const result = await crisisManager.callEmergencyService('suicide_prevention_lifeline');
    ...
    it('maintains functionality when storage fails', async () => {
      ...
      await expect(crisisManager.handleCrisisDetected(crisisData)).resolves.not.toThrow();
      expect(Alert.alert).toHaveBeenCalled();
    });
  });
  ...
  describe('Privacy and Security', () => {
    it('does not store sensitive personal information', async () => {
      ...
      expect(parsedData).not.toContainEqual(expect.stringContaining('John Doe'));
      expect(parsedData).not.toContainEqual(expect.stringContaining('123 Main St'));
      expect(parsedData).not.toContainEqual(expect.stringContaining('john@example.com'));
    });
