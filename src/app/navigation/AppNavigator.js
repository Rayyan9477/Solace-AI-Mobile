...
const AppNavigator = () => {
  ...
  // Add timeout protection for auth checking to prevent infinite loading
  const [authTimeout, setAuthTimeout] = React.useState(false);

  React.useEffect(() => {
    const timeout = setTimeout(() => {
      if (!authChecked) {
        console.warn('🚨 AppNavigator: Auth checking timed out after 5s, forcing completion');
        setAuthTimeout(true);
      }
    }, 5000);
  ...
  // Improved navigation logic with timeout protection
  const renderScreen = () => {
    // If auth hasn't been checked and timeout hasn't occurred, show loading
    ...
    // If still loading (but auth has been checked or timeout occurred), show loading
    ...
    // If timeout occurred, force to authentication flow
    if (authTimeout && !isAuthenticated) {
      console.log('🚨 AppNavigator: Timeout occurred, redirecting to auth');
      return (
        <Stack.Screen
          name="Auth"
          component={AuthStack}
